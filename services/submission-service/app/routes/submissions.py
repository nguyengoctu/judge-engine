...
@router.post("", response_model=SubmissionResponse, status_code=201,
             summary="Submit code", description="Submit code for execution. Returns immediately with pending status.")
def create_submission(body: SubmissionCreate, db: Session = Depends(get_db)):
    submission = Submission(
        id=uuid.uuid4(),
        user_id="anonymous",
        problem_id=uuid.UUID(body.problem_id) if body.problem_id else uuid.uuid4(),
        code=body.code,
...
@router.get("/{submission_id}", response_model=SubmissionResponse,
            summary="Get submission status", description="Poll this endpoint to check execution status.")
